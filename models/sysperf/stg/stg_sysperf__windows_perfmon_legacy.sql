...
with source_data as (
    select 
        *
    from {{ ref('sysperf__windows_perfmon_legacy') }}
...
,   legacy_lookup as (
        SELECT * FROM {{ source('mart_prod','legacy_cc_mapping')}}
    )

,   sysinfo as (
        SELECT * FROM {{ ref('sysperf__windows_sysinfo_legacy')}}
)
...
, map_ids as (
    SELECT
        record_id
        ,m.bubble_clientid as client  -- null client if not in lookup table
        ,coalesce(m.bubble_resourceid,s.cc) as resource  -- cc if not in lookup table
        ,hostname
        ,timestamp
        ,L1
        ,L2
